...
from pytorchtools import EarlyStopping # downloaded from https://github.com/Bjarten/early-stopping-pytorch
from utilities import find_recall_for_fixed_precision

mode = 'masked' # the type of input representation
dataset = JobData(mode=mode)
(Xtrain, Lentrain, Ytrain, widx_ss_train), (Xtest, Lentest, Ytest, widx_ss_test) = \
...
lstm = LSTM(dataset, input_size, hidden_size, num_layers, batch_size=batch_size, dropout=dropout)
...
def get_cv_data(mode=mode, batch_size=512):
    ...
    return test_loader, Lentest, Ntest

test_loader, Lentest, Ntest = get_cv_data(mode=mode, batch_size=512)
